...
int ReadGasData( void )
{
  ...
  //ADCCON3 = 0x86;// 0x87;
  ADCCON3 = (0x40 | HAL_ADC_DEC_256 | HAL_ADC_CHANNEL_6);
  
  /* Wait for the conversion to be done */
  while (!(ADCCON1 & 0x80));
  
  /* Disable channel after done conversion */
  ADCCFG &= (0x80 ^ 0xFF); //按位异或。如1010^1111=0101（二进制）
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  {
    case SAMPLEAPP_PERIODIC_CLUSTERID:    //接收到广播信息
      osal_memset(buf, 0 , 1);
      ...
      if(buf[0]=='w')      //判断收到的数据是否为"w" 
      {
        SampleApp_SendPeriodicMessage(); 
        HalUARTWrite(0, "\r\n", 2);         // 回车换行
        HalUARTWrite(0, "UNUSUAL :", 9);         // 串口提示报警
      }
    ...
    case SAMPLEAPP_FLASH_CLUSTERID:  //接收组播信息
